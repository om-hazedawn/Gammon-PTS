...
@Component({
  ...
  template: `
    <h3 mat-dialog-title>Key Dates</h3>
...
export class TenderKeyDateComponent {
  ...
  handleSubmit(): void {
    if (this.tenderForm.valid) {
      const formData = {
        winningCompetitor: this.tenderForm.value.winningCompetitor || 'N/A',
        marginLost: this.tenderForm.value.marginLost || 0,
        otherReasonForLoss: this.tenderForm.value.otherReasonForLoss || 'N/A'
      };
