...
@Component({
    selector: 'app-generate-snapshot-dialog',
    standalone: true,
    imports: [
        CommonModule,
        ReactiveFormsModule,
    ...
    template: `
        <h1 mat-dialog-title style="color: #1976d2; font-weight: 600;">Generate Snapshot</h1>
...
export class GenerateSnapshotDialogComponent implements OnInit {
    ...
    asOfDate: any[] = [];
    divisions: string[] = [];
    hideDivision: boolean = false;

    constructor(
        public dialogRef: MatDialogRef<GenerateSnapshotDialogComponent>,
        private tenderListApiService: TenderListApiService,
        @Inject(MAT_DIALOG_DATA) public data?: { hideDivision?: boolean }
    ) {
        this.hideDivision = data?.hideDivision ?? false;
    }

    ngOnInit() {
        // Fetch divisions from API only if division field is shown
        ...
        // Fetch period dropdown dates from API
        this.tenderListApiService.periodDropdown().subscribe({
            next: (response) => {
