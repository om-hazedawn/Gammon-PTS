      ...
      <mat-card>
        ...
        <mat-card-content>
          ...
          <form [formGroup]="formGroup" (ngSubmit)="onSubmit()">
            ...
            <app-form-detail-distribution-step [isEditMode]="isEditMode" [formId]="formId"></app-form-detail-distribution-step>
            } @if (currentStep === 11) {
...
export class FormDetailComponent implements OnInit {
  ...
  previousStep(): void {
    if (this.currentStep > 1) {
      // Auto-save before going to previous step
      if (this.formDeleted) {
        // If form is deleted, just go to previous step without saving
        this.currentStep--;
        return;
      }
      ...
      if (this.currentStep === 2 && (!this.formId || this.formId === 0)) {
        ...
        this.form20Service.saveForm20(normalizedValue).subscribe({
          next: (response: number) => {
            console.log('Form updated, formId:', response);
            // Reload form to get latest updates from database
            this.loadForm(this.formId!);
            this.currentStep--;
            this.isLoading = false;
          },
